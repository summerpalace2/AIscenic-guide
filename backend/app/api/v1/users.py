...
async def update_interests(user_id: str, req: UpdateInterestsRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db.commit()
    return {'code': 0, 'success': True, 'message': 'Updated', 'data': {'user_id': str(user.id), 'nickname': user.nickname, 'avatar': user.avatar, 'role': user.role, 'phone': user.phone or '', 'interests': user.interests or [], 'created_at': user.created_at}}
