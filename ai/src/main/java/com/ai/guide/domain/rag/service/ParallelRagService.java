...
public class ParallelRagService {
    ...
    private final ScenicDataImportService dataImportService;
    /** Spring 托管的并行检索线程池，避免每个 Service 实例各自创建线程。 */
    ...
    public ParallelRagService(ScenicDataImportService dataImportService,
                              @Qualifier("parallelRagExecutor") ExecutorService executor) {
    ...
    public String search(List<String> subQueries) {
        ...
        for (Future<List<String>> future : futures) {
            try {
                ...
                // 超时任务不应继续长期占用 RAG 专用线程；底层客户端是否能中断由其实现决定。
                future.cancel(true);
                log.warn("[ParallelRag] 子问题检索超时: {}", e.getMessage());
            } catch (InterruptedException e) {
                future.cancel(true);
                Thread.currentThread().interrupt();
                log.warn("[ParallelRag] 收集子问题检索结果时被中断");
                break;
            } catch (ExecutionException e) {
                log.warn("[ParallelRag] 子问题检索失败: {}", e.getCause() == null ? e.getMessage() : e.getCause().getMessage());
            }
        }
