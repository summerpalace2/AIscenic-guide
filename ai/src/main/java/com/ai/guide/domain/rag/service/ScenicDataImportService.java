...
public class ScenicDataImportService {
    ...
    private static final String PRODUCTION_NAMESPACE = "production-v2";
    ...
    private int insertToQdrant(List<Document> docs, String sourceName) {
        ...
        for (Document doc : docs) {
            ...
            try {
                float[] v = requireValidEmbedding(embeddingModel.embed(cleanText), "入库");
    ...
    private String doQueryKnowledge(String queryText, long timeoutMs) {
        // [步骤1] 向量化查询文本
        float[] queryVector = requireValidEmbedding(embeddingModel.embed(queryText), "检索");
        List<Float> vectorAsList = toFloatList(queryVector);

        // [步骤2] 仅检索 Production V2 且明确允许进入 RAG 的载荷。
        Points.Filter filter = productionSemanticFilter(queryText);
        Matcher idMatcher = idPattern.matcher(queryText.toUpperCase());
        if (idMatcher.find()) {
            log.info("【极速标签匹配】触发意图识别, ID: {}", idMatcher.group());
        }
    ...
    public List<String> searchFragments(String queryText, int qdrantLimit, int rerankTopN) {
        ...
        // [步骤3] Qdrant 向量检索
        log.info("[Qdrant RAG 语义检索] targetCollection={}, queryLength={}, limit={}, rerankTopN={}",
                collectionName(), queryText.length(), qdrantLimit, rerankTopN);
        Points.SearchPoints.Builder searchBuilder = Points.SearchPoints.newBuilder()
                .setCollectionName(collectionName())
                .addAllVector(vectorAsList)
        ...
        try {
            ...
            log.warn("[Qdrant RAG 语义检索失败，将降级] collection={}, error={}", collectionName(), e.getMessage());
            return Collections.emptyList();
        }
    ...
    /**
     * 语义边界统一过滤 Production V2 载荷；旧集合或未标记为 RAG 的点不得进入回答上下文。
     */
    private Points.Filter productionSemanticFilter(String queryText) {
        Points.Filter.Builder builder = Points.Filter.newBuilder()
                .addMust(ConditionFactory.matchKeyword("activation_namespace", PRODUCTION_NAMESPACE))
                .addMust(ConditionFactory.match("rag_eligible", true));
        Matcher matcher = idPattern.matcher(String.valueOf(queryText == null ? "" : queryText).toUpperCase());
        if (matcher.find()) {
            builder.addMust(ConditionFactory.matchKeyword("scenic_id", matcher.group()));
        }
        return builder.build();
    }

    /**
     * 向量提供方失败时必须显式失败；零向量、NaN 和错误维度都不能送入 Qdrant。
     */
    private static float[] requireValidEmbedding(float[] vector, String operation) {
        if (vector == null || vector.length != EMBEDDING_DIMENSION) {
            throw new IllegalStateException("Embedding " + operation + " returned invalid dimension");
        }
        double normSquared = 0d;
        for (float value : vector) {
            if (!Float.isFinite(value)) {
                throw new IllegalStateException("Embedding " + operation + " returned non-finite values");
            }
            normSquared += (double) value * value;
        }
        if (!(normSquared > 0d)) {
            throw new IllegalStateException("Embedding " + operation + " returned a zero vector");
        }
        return vector;
    }
