package com.ai.guide.domain.admin.api;

import com.ai.guide.domain.voice.api.TtsController;
 ...
 * 系统后台全局治理与用户管理控制器
 *
 * 所属领域：domain.admin（管理后台治理域）
 * 架构职责：为管理后台提供系统级用户权限管控、状态冻结以及配置项管理能力（统一进行 ADMIN 权限校验）。
 *
 * 核心对外接口与关键方法：
 * 1. listUsers：分页查询系统已注册用户列表
 *    - 参数：page（页码）、size（每页大小）
 * 2. updateUserRole：调整指定用户的角色权限（如普通用户转管理员）或状态（正常/已禁用）
 *    - 参数：userId（目标用户 ID）、body（包含 role 与 status 的字典）
 * 3. deleteUser：禁用/注销指定用户账号
 * 4. getUserDetail：获取单用户完整资料及注册元数据
 */
@RestController
...
public class AdminController {
    ...
    public Result<Map<String, Object>> getSettings() {
        ...
        // ASR is no longer part of the product surface; hide any legacy row
        // that may have been persisted by an earlier admin-console version.
        data.remove("asrEnabled");
        for (String boolKey : List.of("ttsEnabled", "knowledgeSyncEnabled")) {
